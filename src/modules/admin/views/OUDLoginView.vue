            ...
            <form @submit.prevent="handleLogin" class="mb-3">
              ...
              <div class="mb-3 form-password-toggle">
                ...
                <div class="input-group input-group-merge">
                  <input :type="passwordFieldType" class="form-control" v-model="password" required />
              ...
              <div class="mb-3">
                <button class="btn btn-primary d-grid w-100" type="submit" :disabled="isLoading">{{ isLoading ?
...
const passwordIconClass = ref('bx bx-hide');

function togglePasswordVisibility() {
  if (passwordFieldType.value === 'password') {
    passwordFieldType.value = 'text';
    passwordIconClass.value = 'bx bx-show';
  } else {
    passwordFieldType.value = 'password';
    passwordIconClass.value = 'bx bx-hide';
  }
}
...
const handleLogin = async () => {
  ...
  try {
    ...
    // Após login, defina o perfil do usuário (exemplo: admin)
    // Aqui você pode pegar o perfil do usuário autenticado da authStore
    // Exemplo: userStore.perfil = authStore.user?.perfil || 'admin';
    userStore.perfil = 'admin'; // Ajuste conforme o retorno real da API
    // A própria ação já redireciona em caso de sucesso
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  } catch (err: any) {
    // Se a ação da store lançar um erro, nós o capturamos aqui
    error.value = err.response?.data?.message || 'Credenciais inválidas ou erro de rede.';
  } finally {
